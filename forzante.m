...
% z =  beta(x,y)' * [16 * y * (1-y) * (1-2*x) ; 16 * x * (1 - 2 * y) * (1-x)] + epsilon(x,y) * 32 * (y*(1-y) + x*(1-x)) + gamma(x,y) *( 16 * x*(1-x)*y*(1-y) + 0); % epsilon cost

z =  beta(x,y)' * grad_soluz_esatta(x,y) + epsilon(x,y) * 32 * (y*(1-y) + x*(1-x)) + gamma(x,y) *sol_esatta(x,y);
