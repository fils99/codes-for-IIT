...
global Nt L_q zita csi eta omega geom N_dof U_gdl  grad_phi  u phi_1 phi_2 phi_3
global D C R Se_h
lung = 7;
...
for e=1:Nt
    ...
    for q=1: L_q % ciclo sui nodi di quadratura
        ...
        sol_ex_valutata = sol_esatta_bis(nodo_q_globale(1),  nodo_q_globale(2)); % sol_esatta o sol_esatta_bis
        grad_sol_ex_valutato = grad_soluz_esatta_bis ( nodo_q_globale(1),  nodo_q_globale(2)); % grad_soluz_esatta o grad_soluz_esatta_bis

        sum_approx = 0;
        grad_approx = 0;
        for k = 1:3
            kk =  geom.elements.triangles(e,k);
                sum_approx = sum_approx + u(kk) * phi(k);
                grad_approx = grad_approx +   u(kk) * grad_phi(:,k);
        end
...
salvataggio = round([lista_lati_max, err_L2, err_H1, err_L_inf, numero_condiz_stiff],4);

% save dati.txt salvataggio -ascii

% save errore_L2.txt err_L2 -ascii
% save errore_L_inf.txt err_L_inf -ascii
% save errore_H1.txt err_H1 -ascii
% save condiz.txt numero_condiz_stiff -ascii


% figure(4) altro possibile plot degli errori
% plot(log(lista_lati_max), Poli_L2 )
% axis equal
% hold on
% plot(log(lista_lati_max), Poli_H1 )
% axis equal
% hold on
% plot(log(lista_lati_max), Poli_L_inf )
% axis equal
