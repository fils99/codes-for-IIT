function assembla_matrice () % P1 ellittica
...
global tau_h R_tot_lumping D C R Se_h 
...
for e = 1 : Nt
    ...
    f_bar = forzante (x_bar, y_bar); % valuto forzante nel baricentro

    Pe_h = m_k * norm(beta_bar) * h_E / (2 * epsilon_bar);

    Se_h = gamma_bar * h_E^2 / (6 * epsilon_bar);
            ...
            for k = 1 : 3
                ...
                L_q = length(omega);
                    ...
                    for q = 1: L_q
                        ...
                        term_noto  = term_noto  + omega(q) * forzante_bis(nodo_q_globale(1), nodo_q_globale(2)) * phi(j) * 2 * area; % forzante o forzante_bis
                        ...
                        stab_termine_noto = stab_termine_noto + tau_h * omega(q) * forzante_bis (nodo_q_globale(1), nodo_q_globale(2))  * beta ( nodo_q_globale(1), nodo_q_globale(2))' * B_inv_tras  * grad_phi(:,j) * (2* area); % forzante o forzante_bis
...
% decommentare D_stab, C_stab e R_stab se si vuole attivare
% stabilizzazione SUPG
D_tot = D;% + D_stab;
C_tot = C;% + C_stab;
R_tot = R;% + R_stab;

% mass lumping sulla matrice di reazione
somme_righe_R = sum(R_tot); 
R_tot_lumping = diag (somme_righe_R); 
R_tot_lumping = R_tot;  % commentare questa riga se si vuole utilizzare il mass lumping

A_tot_lumping = D_tot + C_tot + R_tot_lumping;

% decommentare D_D_stab, C_D_stab e R_D_stab se si vuole attivare
% stabilizzazione SUPG
...
b_tot = b + b_N;% + b_stab; % decommentare b_stab  per attivare stabiliz SUPG
